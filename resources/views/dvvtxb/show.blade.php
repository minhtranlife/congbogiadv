...
@section('content')
    ...
    @else
        <h3>Đơn vị cung cấp dịch vụ vận tải xe buýt chưa có kê khai giá gần nhất cho {{(getGeneralConfigs()['tendonvi'])}}</h3>
    @endif

    <div class="row">
        <div class="col-lg-12">
            <h3 class="page-header">Đơn vị cung cấp dịch vụ vận tải cùng loại</h3>
        </div>
        @foreach($modelk as $dnk)
        <div class="col-md-3 portfolio-item">
            <a href="{{url('dn-dich-vu-van-tai-xe-buyt/'.$dnk->masothue)}}">
                @if($dnk->toado != null)
                    <img class="img-responsive"
                         src="https://maps.googleapis.com/maps/api/staticmap?zoom=17&amp;size=750x425&amp;sensor=false&amp;
            ...
            <h3><a href="{{url('dn-dich-vu-van-tai-xe-buyt/'.$dnk->masothue)}}">{{$dnk->tendonvi}}</a></h3>
